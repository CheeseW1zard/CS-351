...
int x = blockIdx.x*blockDim.x + threadIdx.x;
    ...
    // 2) reconstruct the lower-left corner
    Complex ll = Complex(-center.x, -center.y);

    // 3) map pixel -> complex plane: z0 = ll + (x,y)*d
    Complex z{ ll.x + x*d.x,
               ll.y + y*d.y };

    // 4) iterate z = z^2 + c
    int iter = 0;
    const int maxIter = MaxIter;   // should be defined in your headers
    while (iter < maxIter && magnitude(z) <= 2.0f) {
        z = z*z + center;
        ++iter;
    }
pixels[idx] = setColor(iter, MaxIter);
